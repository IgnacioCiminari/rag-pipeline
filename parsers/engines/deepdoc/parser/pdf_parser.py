...
from deepdoc.compat import MAXIMUM_PAGE_NUMBER, get_project_base_directory, rag_tokenizer, vision_llm_describe_prompt, settings, thread_pool_exec
...
class RAGFlowPdfParser:
    def __init__(self, **kwargs):
        ...
        try:
            model_dir = os.path.join(get_project_base_directory(), "deepdoc/res")
            self.updown_cnt_mdl.load_model(os.path.join(model_dir, "updown_concat_xgb.model"))
        except Exception:
            model_dir = snapshot_download(repo_id="InfiniFlow/text_concat_xgb_v1.0", local_dir=os.path.join(get_project_base_directory(), "deepdoc/res"), local_dir_use_symlinks=False)
            self.updown_cnt_mdl.load_model(os.path.join(model_dir, "updown_concat_xgb.model"))
...
class VisionParser(RAGFlowPdfParser):
    ...
    def __call__(self, filename, from_page=0, to_page=MAXIMUM_PAGE_NUMBER, **kwargs):
        ...
        for idx, img_binary in enumerate(self.page_images or []):
            ...
            from deepdoc.compat import vision_llm_chunk as picture_vision_llm_chunk
