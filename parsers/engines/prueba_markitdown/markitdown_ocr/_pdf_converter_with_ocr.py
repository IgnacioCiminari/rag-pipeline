...
from markitdown.converters._pdf_converter import _calculate_global_margins, _crop_page
        ...
        **kwargs: Any,
        ...
        try:
            with pdfplumber.open(pdf_bytes) as pdf:
                logging.info(f"[OCR] PDF abierto exitosamente. Total de páginas: {len(pdf.pages)}")
                
                global_header_y, global_footer_y = _calculate_global_margins(pdf)
                logging.info(f"[OCR] Consenso de márgenes globales calculado: Header Y={global_header_y}, Footer Y={global_footer_y}")
                
                for page_num, page in enumerate(pdf.pages, 1):
                    logging.info(f"[OCR] Analizando página {page_num}...")
                    ...
                    if ocr_service:
                        logging.info(f"[OCR] Extrayendo imágenes de la página {page_num}...")
                        images_on_page = _extract_images_from_page(cropped_page)
                        images_on_page.sort(key=lambda x: x["y_pos"])
                        if images_on_page:
                            logging.info(f"[OCR] Se encontraron {len(images_on_page)} imágenes en la página {page_num}.")
                        else:
                            logging.info(f"[OCR] No se detectaron imágenes en la página {page_num}.")
                        ...
                        if images_on_page:
                            ...
                            if chars:
                                ...
                                text_content = cropped_page.extract_text() or ""
                                lines_with_y = [
                                    {"y": i * 10, "text": line}
                            ...
                            for img_info in images_on_page:
                                logging.info(f"[OCR] Consultando modelo de lenguaje (OpenAI) para la imagen: {img_info['name']}")
                                ocr_result = ocr_service.extract_text(
                                    img_info["stream"]
                                )
                                if ocr_result.text.strip():
                                    logging.info(f"[OCR] Resultado LLM: Se extrajeron {len(ocr_result.text.strip())} caracteres.")
                                    image_data.append(
                                        {
